...
//!
//! 输出约定（脚本可解析）：`== step` 步骤开始、`ok step` 步骤完成、
//! `FAILED: ...` / `ROLLBACK ...`（stderr）、末行 `Install complete: <dir>` /
//! `Uninstall complete (purge=...)`（stdout）。绝不打印任何凭据。
...
fn run_uninstall(args: &SetupArgs) -> Result<()> {
    ...
    work::kill_tray();
    // purge 只在显式 --purge 时为 true；卸载核心幂等宽容。
    crate::service::uninstall_core(&config_path(), args.purge)?;
