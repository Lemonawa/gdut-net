...
- **平台分层**：纯逻辑（`backoff`/`cmdline`/`config`/`heartbeat::spec`/`ipc::protocol`/`watchdog`/`probe` 判定函数/`wireless` 的 Brain、portal 与 Wireless Egress 纯半/`supervisor::notification`/`setup::transaction`/`payload`/`packaging`/`setup_args`/`shell_shortcuts`）无 `windows::` 依赖，Linux 可跑 TDD；Win32 胶水（`ras`/`adapter`/`service`/`runtime`/`eventlog`/`notify`/`tray`（`mod.rs` 托盘本体 + `gui.rs` 日常窗口）/`shell.rs`（开始菜单/卸载键）/`setup::{ui,work}`/`wireless::{routes,wlan,test}`）仅 `cfg(windows)`，靠交叉编译验证。
