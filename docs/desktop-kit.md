...
| 回家模式（管理员） | 停服务+设手动+退托盘+**摘 HKCU Run 自启**+**停用 Clash 校园 DNS 策略**（家里不可达、每次查询白等 5s；需重启 Verge 生效）；不碰系统代理。托盘另带守卫：服务"按需+未运行"时启动即退出，所以就算被 Win11 的"重启后自动重新打开应用"之类拉起来也不会常驻；回校模式全部恢复 |
| 启动托盘 | 托盘被杀后重新拉起 |
